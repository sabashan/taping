...
public class FileUploadController extends BaseFormController {
	...
	@Autowired
	public void setManager(MstpictureManager manager) {
		this.mstpictureManager = manager;
	}
    ...
    public String onSubmit(FileUpload fileUpload, BindingResult errors, HttpServletRequest request)
        ...
        if (request.getParameter("cancel") != null) {
        	...
        	log.debug("Delete");
        	String pic =request.getParameter("picid");
        	if(!StringUtils.isBlank(pic))
        		try{
        			mstpictureManager.remove(pic);
        		}catch (Exception e) {
					// TODO: handle exception
				}
        	else
        		log.debug("Delete else ");
        }
        else{
	        ...
	        if (fileUpload.getName() == null) {
	        	...
	        	//Create img file name with current date time + img file name
	        	System.out.println(dateFormat.format(cal.getTime())+"_"+file.getOriginalFilename());
	        ...
	        String path = dirPath.getAbsolutePath() + Constants.FILE_SEP + file.getOriginalFilename();
	        ...
	        log.debug(fileUpload.getName() + " :" + file.getOriginalFilename());
	        System.out.println("*********************************************"+ bFile);
	        Mstpicture mstpicture = new Mstpicture(fileUpload.getName(), file.getOriginalFilename(), bFile);
	        System.out.println("*********************************************");
	        mstpictureManager.save(mstpicture);
	        System.out.println("*********************************************");
