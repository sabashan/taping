package com.excelsoft.webapp.listener;
...
public class UserCounterListener implements ServletContextListener, HttpSessionAttributeListener, HttpSessionListener {
    ...
    synchronized void addUsername(Mstuser user) {
        users = (Set<Mstuser>) servletContext.getAttribute(USERS_KEY);

        if (users == null) {
            users = new LinkedHashSet<Mstuser>();
        }
    ...
    @SuppressWarnings("unchecked")
    synchronized void removeUsername(Mstuser user) {
        users = (Set<Mstuser>) servletContext.getAttribute(USERS_KEY);
    ...
    public void attributeAdded(HttpSessionBindingEvent event) {
        if (event.getName().equals(EVENT_KEY) && !isAnonymous()) {
            ...
            if (securityContext != null && securityContext.getAuthentication().getPrincipal() instanceof Mstuser) {
                Mstuser user = (Mstuser) securityContext.getAuthentication().getPrincipal();
                addUsername(user);
            }
        }
    }
    ...
    public void attributeRemoved(HttpSessionBindingEvent event) {
        if (event.getName().equals(EVENT_KEY) && !isAnonymous()) {
            ...
            if (auth != null && (auth.getPrincipal() instanceof Mstuser)) {
                Mstuser user = (Mstuser) auth.getPrincipal();
                removeUsername(user);
            }
        }
    }
    ...
    public void attributeReplaced(HttpSessionBindingEvent event) {
        if (event.getName().equals(EVENT_KEY) && !isAnonymous()) {
            ...
            if (securityContext.getAuthentication() != null
                    && securityContext.getAuthentication().getPrincipal() instanceof Mstuser) {
                final Mstuser user = (Mstuser) securityContext.getAuthentication().getPrincipal();
                addUsername(user);
            }
        }
    }
