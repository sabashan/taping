...
public class UserSecurityAdvice implements MethodBeforeAdvice, AfterReturningAdvice {
     ...
     * @throws Throwable thrown when args[0] is null or not a Mstuser object
     */
    public void before(Method method, Object[] args, Object target) throws Throwable {
        ...
        if (ctx.getAuthentication() != null) {
            ...
            if (!signupUser) {
                ...
                Mstuser currentUser = getCurrentUser(auth, userManager);
                ...
                if (user.getUserId() != null && !user.getUserId().equals(currentUser.getUserId()) && !administrator) {
                    ...
                    Set<String> userRoles = new HashSet<String>();
                            userRoles.add(user.getUserGroup());
                    ...
                    if (!CollectionUtils.isEqualCollection(user.getAuthorities(), authorizedRoles)) {
    ...
    private Mstuser getCurrentUser(Authentication auth, UserManager userManager) {
        Mstuser currentUser;
        if (auth.getPrincipal() instanceof LdapUserDetails) {
            ...
            currentUser = (Mstuser) auth.getPrincipal();
        } else if (auth.getDetails() instanceof UserDetails) {
            currentUser = (Mstuser) auth.getDetails();
        } else {
            throw new AccessDeniedException("Mstuser not properly authenticated.");
        }
        return currentUser;
    }
}
